...
public class ERC20 {
    ...
    public func name(tokenContract: EthereumAddress, completion: @escaping((Error?, String?) -> Void)) {
        let function = ERC20Functions.name(contract: tokenContract.address)
    ...
    public func symbol(tokenContract: EthereumAddress, completion: @escaping((Error?, String?) -> Void)) {
        let function = ERC20Functions.symbol(contract: tokenContract.address)
    ...
    public func decimals(tokenContract: EthereumAddress, completion: @escaping((Error?, BigUInt?) -> Void)) {
        let function = ERC20Functions.decimals(contract: tokenContract.address)
    ...
    public func balanceOf(tokenContract: EthereumAddress, address: EthereumAddress, completion: @escaping((Error?, BigUInt?) -> Void)) {
        let function = ERC20Functions.balanceOf(contract: tokenContract.address, account: address.address)
    ...
    /*
    public func transferEventsTo(recipient: String, fromBlock: EthereumBlock, toBlock: EthereumBlock, completion: @escaping((Error?, [ERC20Events.Transfer]?) -> Void)) {
        
        guard let addressType = ABIRawType(type: String.self), let result = try? ABIEncoder.encode(recipient, forType: addressType), let sig = try? ERC20Events.Transfer.signature() else {
